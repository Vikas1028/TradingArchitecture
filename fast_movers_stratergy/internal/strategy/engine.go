...
func (e *Engine) emit(strategyName, symbol, side string, now time.Time, reason string, stopLoss, target float64) common.Signal {
	e.signalsTotal++
	switch strategyName {
	case "fast_movers_stratergy_s1_open_reclaim":
		e.strategy1Total++
	case "fast_movers_stratergy_s2_30sec_burst":
		e.strategy2Total++
	case "fast_movers_stratergy_s3_two_candle":
		e.strategy3Total++
	case "fast_movers_stratergy_s4_orb_retest":
		e.strategy4Total++
	}
