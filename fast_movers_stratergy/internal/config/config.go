...
func applyDefaults(cfg *common.AppConfig) {
	...
	if strings.TrimSpace(cfg.Kafka.TradesGroupID) == "" {
		cfg.Kafka.TradesGroupID = common.DefaultTradesGroupID
	}
	...
	if cfg.Strategy.MaxOpenTrades <= 0 {
		cfg.Strategy.MaxOpenTrades = 3
	}
...
func validate(cfg *common.AppConfig) error {
	...
	if strings.TrimSpace(cfg.Kafka.TradesGroupID) == "" {
		missing = append(missing, "kafka.trades_group_id")
	}
	if len(missing) > 0 {
