...
// SetWebsocketConnected updates the in-memory and Prometheus websocket connection state.
...
// SetKafkaConnected updates the in-memory and Prometheus Kafka connection state.
...
// SetPostgresConnected updates the in-memory and Prometheus PostgreSQL connection state.
...
// SetClientLoggedIn updates the in-memory and Prometheus broker-login state.
...
// SetSubscriptions stores the active subscription basket for the dashboard snapshot.
...
// SetKafkaQueueDepth records the current Kafka queue depth for metrics and dashboards.
...
// SetPostgresQueueDepth records the current PostgreSQL queue depth for metrics and dashboards.
...
// RecordTick tracks one successfully processed tick across totals, symbols, and recent history.
...
// RecordKafkaInserted increments Kafka delivery totals.
...
// RecordPostgresInserted increments PostgreSQL insert totals.
...
// RecordKafkaReconnect increments Kafka reconnect counters.
...
// RecordPostgresReconnect increments PostgreSQL reconnect counters.
...
// RecordWebsocketReconnect increments websocket reconnect counters.
...
// RecordClientRelogin increments broker re-login counters.
...
// RecordError increments the labeled runtime error counter for one component.
...
// recentSecondCount returns the freshest per-second tick count from the rolling minute map.
