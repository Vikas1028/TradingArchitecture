...
// initializeLogger resolves config, creates the rolling file sink, and builds the zap logger.
func initializeLogger() error {
...
// loadLoggerConfig decodes logger.json into the shared logging config structure.
...
// buildEncoder selects console or JSON encoding for the zap core.
...
// normalizeLevel converts free-form config text into one supported log level.
...
// writeLog routes one formatted log entry to the underlying sugared logger.
...
// shouldLog reports whether the current logger configuration allows the requested level.
