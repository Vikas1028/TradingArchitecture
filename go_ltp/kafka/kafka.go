...
// PublishMessage wraps each LTP snapshot tick and enqueues it for asynchronous
...
func PublishMessage(ctx context.Context, key string, payload common.KafkaTickMessage) error {
	...
	message := outboundMessage{
		subject: tickSubject(appConfig.GlobalConfig.Kafka.Topic, "go_ltp", payload.Symbol),
		payload: body,
		key:     payload.Symbol,
	}
...
// prefix so the poller can publish symbol-scoped tick subjects safely.
