...
func NewCandleConsumer(cfg cfgpkg.KafkaConfig, logger *zap.Logger) (*CandleConsumer, error) {
	...
	stockOpts := []nats.SubOpt{
		nats.BindStream(streamName(cfg.StockCandlesTopic)),
		nats.ManualAck(),
		nats.AckExplicit(),
	}
	indexOpts := []nats.SubOpt{
		nats.BindStream(streamName(cfg.IndexCandlesTopic)),
		nats.ManualAck(),
		nats.AckExplicit(),
	}
	if !startupCutoff.IsZero() {
		stockOpts = append(stockOpts, nats.StartTime(startupCutoff))
		indexOpts = append(indexOpts, nats.StartTime(startupCutoff))
	} else {
		stockOpts = append(stockOpts, nats.DeliverNew())
		indexOpts = append(indexOpts, nats.DeliverNew())
	}
