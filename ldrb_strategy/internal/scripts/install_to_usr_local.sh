#!/usr/bin/env bash
# Build and deploy ldrb_strategy into /Users/vikasbhandekar/live_services on macOS.
...
mkdir -p "${STAGING_DIR}/cmd" "${STAGING_DIR}/config" "${STAGING_DIR}/logs" "${STAGING_DIR}/scripts" "${STAGING_DIR}/data"
cp "${STAGING_DIR}/${APP_NAME}" "${STAGING_DIR}/cmd/${APP_NAME}"
cp "${APP_DIR}/config/ldrb_strategy_config.json" "${STAGING_DIR}/config/ldrb_strategy_config.json"
cp "${APP_DIR}/data/events_next_day.json" "${STAGING_DIR}/data/events_next_day.json"
cp "${APP_DIR}/data/volume_profile_10d.json" "${STAGING_DIR}/data/volume_profile_10d.json"
cp "${APP_DIR}/internal/scripts/ldrb_strategy.properties" "${STAGING_DIR}/scripts/ldrb_strategy.properties"
cp "${APP_DIR}/internal/scripts/ldrb_strategy.plist" "${STAGING_DIR}/scripts/ldrb_strategy.plist"
